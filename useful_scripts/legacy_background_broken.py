...
class PlotGenerationWorker(threading.Thread):
    ...
    def __init__(self, viewer):
        ...
        self.viewer = viewer
        self.request_queue = PriorityQueue()  # (priority, record_id, plot_type)
        self.running = True
        self.max_generated_index = max(2000, int(os.environ.get("SPECLOC_BG_INDEX_MAX", "50000")))
        self.save_every = max(10, int(os.environ.get("SPECLOC_BG_SAVE_EVERY", "200")))
        self.generated_index = self._load_generated_index()
        self._pending_keys = set()
        self._generated_updates = 0
        self.lock = threading.Lock()
        self.exploration_depth = 0
        self.has_pending_user_requests = False
        ...
        if index_file.exists():
            try:
                with open(index_file) as f:
                    data = json.load(f)
                    loaded = data.get("generated", [])
                    if not isinstance(loaded, list):
                        return set()
                    # Keep only a bounded tail to avoid unbounded RAM growth.
                    return set(loaded[-self.max_generated_index:])
            except Exception:
                return set()
        return set()
    ...
    def _save_generated_index(self):
        ...
        index_file = self.viewer.cache_dir / "background_generated_index.json"
        try:
            entries = list(self.generated_index)
            if len(entries) > self.max_generated_index:
                entries = entries[-self.max_generated_index:]
            with open(index_file, 'w') as f:
                json.dump({"generated": entries}, f)
        except Exception:
            pass

    def _mark_generated(self, keys):
        """Track generated keys with bounded memory and periodic persistence."""
        updated = False
        for key in keys:
            if key in self.generated_index:
        ...
        # Trim set if it grows beyond configured bound.
        if len(self.generated_index) > self.max_generated_index:
            # Keep arbitrary bounded subset; exact historical order is not required.
            self.generated_index = set(list(self.generated_index)[-self.max_generated_index:])

        if updated and self._generated_updates >= self.save_every:
    ...
    def request_plot(self, record_id, plot_type, priority=0):
        ...
        if key in self.generated_index or key in self._pending_keys:
            return
        self._pending_keys.add(key)
        self.request_queue.put((priority, record_id, plot_type))
        if priority == 0:
            with self.lock:
                self.has_pending_user_requests = True

    def flush_background_requests(self):
        """Clear low-priority background requests, keep only user requests."""
        temp_queue = PriorityQueue()
        user_requests = []
        
        # Extract all user requests (priority 0)
        while not self.request_queue.empty():
            try:
                priority, record_id, plot_type = self.request_queue.get_nowait()
                if priority == 0:
                    user_requests.append((priority, record_id, plot_type))
            except:
                break
        
        # Re-queue only user requests
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # Drain all high-priority (user) requests from queue
                while not self.request_queue.empty():
                    try:
                        priority, record_id, plot_type = self.request_queue.get_nowait()
                        if priority == 0:  # User request
                            user_req_batch.append((record_id, plot_type))
                        else:
                            # Put background request back if we're not processing them now
                            self.request_queue.put((priority, record_id, plot_type))
                            break
                    except:
                        break
                
                # Process all user requests in a batch
                if user_req_batch:
                    to_render = []
                    for record_id, plot_type in user_req_batch:
                        key = f"{record_id}:{plot_type}"
                        if key not in self.generated_index:
                            to_render.append((record_id, plot_type))
                    
                    if to_render:
                        self.viewer._render_pairs(to_render)
                        self._mark_generated([f"{record_id}:{plot_type}" for record_id, plot_type in to_render])
                ...
                time.sleep(0.05)  # Small delay to allow queue to fill
    ...
    def _explore_background(self):
        ...
        # Generate plots at current neighbors for current depth.
        candidates = []
        ...
        if self.exploration_depth == 0:
            ...
            for target in neighbor_targets[:100]:
                for plot_type in self.viewer.display_plot_types:
                    rec = self.viewer._best_record_for_type(plot_type, target)
                    if rec is not None:
                        candidates.append((rec["record_id"], plot_type))
        ...
        if to_render:
            batch = to_render[:50]
            self.viewer._render_pairs(batch)  # Batch render up to 50
            self._mark_generated([f"{record_id}:{plot_type}" for record_id, plot_type in batch])

        # Move to next depth after each pass; pause briefly at cycle wrap.
    ...
    def stop(self):
        ...
        self.running = False
        self._generated_updates = self.save_every
        self._save_generated_index()
...
class LazyPlotViewer(QMainWindow):
    def __init__(self, source_path):
        ...
        self.rendered_cache_max_entries = max(2000, int(os.environ.get("SPECLOC_LAZY_RENDERED_CACHE_MAX", "50000")))
        ...
        # Start background plot generation worker only when enabled.
        default_bg = "0" if len(self.records) > 100000 else "1"
        self.background_worker_enabled = os.environ.get("SPECLOC_BG_WORKER", default_bg).lower() not in ("0", "false", "no")
        if self.background_worker_enabled:
            self.plot_worker = PlotGenerationWorker(self)
            self.plot_worker.start()
            print("Background plot worker started")
        else:
            self.plot_worker = None
            print("Background plot worker disabled")
    ...
    def _render_variant(self, plot_type, target_params=None):
        ...
        if plot_type in ("specloc_signature_vs_E", "specloc_loggap_vs_E"):
            return f"Eref:{self._current_target_E}:Gref:{gamma_tag}"
    ...
    def _trim_rendered_path_cache(self):
        if len(self.rendered_path_cache) <= self.rendered_cache_max_entries:
            return
        drop_count = len(self.rendered_path_cache) - self.rendered_cache_max_entries
        for _ in range(drop_count):
            try:
                self.rendered_path_cache.pop(next(iter(self.rendered_path_cache)))
    ...
    def _render_pairs(self, pairs, target_params=None):
        ...
        for pair in pairs:
            ...
            if self._path_exists_and_nonempty(cached_path):
                ...
                if ptype in ("specloc_signature_vs_E", "specloc_loggap_vs_E"):
                    missing_has_siggap = True
        ...
        gamma_ref = target_params.get("gamma", float("nan"))
        if not math.isnan(float(gamma_ref)):
            env["SPECLOC_SIGGAP_G_REF"] = str(float(gamma_ref))
        ...
        if missing_has_spectrum or missing_has_siggap:
        ...
        try:
            ...
            self._trim_rendered_path_cache()
    ...
    def _toggle_perturbation(self):
        ...
        # Flush background requests to prioritize this user change
        if hasattr(self, 'plot_worker'):
            self.plot_worker.flush_background_requests()
        self._update_toggle_labels()
        self.update_display()
    ...
    def _format_record_metadata(self, rec, plot_type, target_params=None):
        if rec is None:
            return "Record: none"

        if target_params is None:
            target_params = self._last_target_params or self.current_params

        metadata_overrides = {}
        if plot_type in ("specloc_signature_vs_E", "specloc_loggap_vs_E", "specloc_spectrum_vs_gamma"):
            g = target_params.get("gamma", float("nan"))
            if not math.isnan(float(g)):
                metadata_overrides["gamma"] = float(g)

        rid = str(rec.get("record_id", "?"))
        keys = ["A", "B", "m", "gamma", "W", "kappa", "E"]
        parts = []
        for k in keys:
            v = metadata_overrides.get(k, rec.get(k, None))
    ...
    def closeEvent(self, event):
        ...
        if hasattr(self, 'plot_worker'):
            ...
            self.plot_worker.join(timeout=2)
    ...
    def update_display(self):
        ...
        # Queue user-requested plots with high priority (0) to the background worker
        if hasattr(self, 'plot_worker'):
            for record_id, plot_type in selected:
                self.plot_worker.request_plot(record_id, plot_type, priority=0)
        ...
        for plot_type, card in self.plot_cards.items():
            ...
            card["meta"].setText(self._format_record_metadata(rec, plot_type, target_params=target_params))
