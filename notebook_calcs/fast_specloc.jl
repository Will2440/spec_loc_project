...
function compute_low_lying_localiser_Krylov(
    L::SparseMatrixCSC{ComplexF64, Int};
    n_eigenpairs::Int=10,
    kk_tol::Real=1e-8,
    kk_maxiter::Int=1000
)::Vector{Float64}
    # Matrix-free operator for L^2
    L2_action(x) = L * (L * x)
    n = size(L, 1)
    
    # Request an even number of eigenpairs to capture (+λ, -λ) pairs symmetric in L^2
    n_pairs = isodd(n_eigenpairs) ? n_eigenpairs + 1 : n_eigenpairs
    
    evals_L2, evecs, info = eigsolve(L2_action, rand(ComplexF64, n), n_pairs, :SR; 
                                     ishermitian=true, tol=kk_tol, maxiter=kk_maxiter)
    
    # Form basis V and ensure closure under L by including L*V
    V_raw = hcat(evecs...)
    LV_raw = L * V_raw
    
    # Orthonormalize extended subspace [V L*V] for exact Rayleigh-Ritz projection
    Q = qr(hcat(V_raw, LV_raw)).Q
    V_sub = Matrix(Q[:, 1:size(hcat(V_raw, LV_raw), 2)])
    
    # Rayleigh-Ritz projection onto invariant subspace
    H_sub = V_sub' * (L * V_sub)
    signed_evals = eigvals(Hermitian(H_sub))
    
    # Sort by absolute magnitude and return requested count
...
function fast_low_lying_localiser_spectrum(
    ...
    for Lx in Lxs, Ly in Lys
        ...
        @showprogress dt=0.1 Threads.@threads for i in 1:n_params
            ...
            # Accumulate eigenvalues over disorder realisations
            low_lying_evals_accum = zeros(Float64, n_lowest_evals)
            ...
            for realisation in 1:n_disorder_realisations
                ...
                # Accumulate eigenvalues (handles cases where length might vary)
                for j in 1:min(length(low_lying_evals), n_lowest_evals)
                    low_lying_evals_accum[j] += low_lying_evals[j]
                end
            end
            
            # Average over realisations
            low_lying_evals_avg = low_lying_evals_accum ./ n_disorder_realisations
...
Bvals = [1.0]
mvals = [-1.0]
gammavals = [0.0] #, 0.5, 1.0, 2.0, 3.0] #collect(0.0:0.1:3.0) #collect(range(0.0, 2.0, 51))
perturbation_type = :symmetric
disorder_type = :anderson
Ws = collect(range(0.0, 2.0, 21))
n_disorder_realisations = 20
Lxs = [20] #collect(5:5:30) #[14]
Lys = [20] #collect(5:5:30) #[14]
keep_square = true
xs = :centre
ys = :centre
sweep_x0y0 = (false, 51)
Es = collect(range(0.0, 2.0, length=51)) #collect(range(-1.125, -1.075, length=101))
kappas = [0.02] #logrange(1e-3, 1e-0, 3) #collect(range(1e-3, 1e-0, 30)) #[2e-1]
n_lowest_evals = 4
...
results_df = fast_low_lying_localiser_spectrum(
    ...
    n_lowest_evals=n_lowest_evals, sparse_output=true,
